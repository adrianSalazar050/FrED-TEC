...
def encontrar_puerto_arduino():
    ...
    if platform.system() == "Windows": return "COM9"
...
class ControlGUI(QWidget):
    def __init__(self):
        ...
        actuator_names = ["Motor DC", "Fan", "Extrusor", "Heater"]
        ...
        self.btn_spool = QPushButton("Motor DC")
        self.btn_fan = QPushButton("Fan")
        self.btn_extrude = QPushButton("Extrusor")
        self.btn_heater = QPushButton("Heater")
        for idx, btn in enumerate([self.btn_spool, self.btn_fan, self.btn_extrude, self.btn_heater]):
        ...
        self.le_kp_h = QLineEdit("12.0")
        self.le_ki_h = QLineEdit("0.9")
        self.le_kd_h = QLineEdit("1.5")
        ...
        self.le_kp_m = QLineEdit("7.5")
        self.le_ki_m = QLineEdit("2.2")
        self.le_kd_m = QLineEdit("0.3")
    ...
    def actualizar_indicador_estado(self, index):
        ...
        actuator_name = text_label.text().split(':')[0]
    ...
    def actualizar(self):
        ...
        self.motor_rpm_data = self.motor_rpm_data[-max_len:]
        self.grosor_data = self.grosor_data[-max_len:]
        self.motor_dc_state_data = self.motor_dc_state_data[-max_len:]
        self.fan_state_data = self.fan_state_data[-max_len:]
        self.extruder_state_data = self.extruder_state_data[-max_len:]
        self.heater_state_data = self.heater_state_data[-max_len:]
        
        self.canvas_temp.plot(self.temp_data, ylabel="Temp (°C)")
        self.canvas_motor.plot(self.motor_rpm_data, ylabel="Motor DC (RPM)")
        self.canvas_grosor.plot(self.grosor_data, ylabel="Grosor (mm)")

    # ##################################################################
    # # INICIO: FUNCIÓN DE EXPORTAR CSV ACTUALIZADA                    #
    # ##################################################################
    def export_csv(self):
        ...
        # Determinar el número máximo de filas basado en la lista más larga de datos
        all_data = [
    ...
    # ##################################################################
    # # FIN: FUNCIÓN DE EXPORTAR CSV ACTUALIZADA                       #
    # ##################################################################
